...
describe("SystemTesting9", function () {
    ...
    before("Initial set up", async function () {
        ...
        await factory.deployGumBall('GBT1', 'GBT1', ['testuri', 'testURI'], oneHundred, oneHundred, weth.address, artist.address, 7*24*3600, [50, 50]);
    ...
    it('User1 Tries to Buy GBT with 10 WETH', async function () {
        console.log("******************************************************");
        await weth.connect(user1).approve(GBT.address, ten);
        await expect(GBT.connect(user1).buy(ten, 1, 1782282187, AddressZero)).to.be.revertedWith("Allowlist amount overflow");
    });

    it('User2 Tries to Buy GBT with 10 WETH', async function () {
        console.log("******************************************************");
        await weth.connect(user2).approve(GBT.address, ten);
        await expect(GBT.connect(user2).buy(ten, 1, 1782282187, AddressZero)).to.be.revertedWith("Allowlist amount overflow");
    });

    it('User3 Tries to Buy GBT with 10 WETH', async function () {
        console.log("******************************************************");
        await weth.connect(user3).approve(GBT.address, ten);
        await expect(GBT.connect(user3).buy(ten, 1, 1782282187, AddressZero)).to.be.revertedWith("Allowlist amount overflow");
    });

    it('Owner adds User3 to allowlist with 10', async function () {
        console.log("******************************************************");
        await expect(factory.connect(user3).updateGumBallAllowlist(GBT.address, [user3.address], ten)).to.be.reverted;
        await factory.connect(owner).updateGumBallAllowlist(GBT.address, [user3.address], ten)
    });

    it('User3 Buys GBT with 10 WETH', async function () {
        console.log("******************************************************");
        await weth.connect(user3).approve(GBT.address, ten);
        await GBT.connect(user3).buy(ten, 1, 1782282187, AddressZero);
    });
    ...
    it('User3 Tries to Buy more GBT with 10 WETH', async function () {
        console.log("******************************************************");
        await weth.connect(user3).approve(GBT.address, ten);
        await expect(GBT.connect(user3).buy(ten, 1, 1782282187, AddressZero)).to.be.revertedWith("Allowlist amount overflow");
    });

    it('Artist adds User3 to allowlist with 10 more GBT', async function () {
        console.log("******************************************************");
        await expect(GBT.connect(owner).updateAllowlist([user3.address], ten)).to.be.reverted;
        await GBT.connect(artist).updateAllowlist([user3.address], ten)
    });
    ...
    it('User3 Buys GBT with 100 WETH', async function () {
        ...
        await expect(GBT.connect(user3).buy(oneHundred, 1, 1782282187, AddressZero)).to.be.revertedWith("Allowlist amount overflow");
    });

    it('Artist adds User1 to allowlist with 10 more GBT', async function () {
        console.log("******************************************************");
    ...
    it('User2 Buys GBT with 10 WETH', async function () {
        console.log("******************************************************");
        await weth.connect(user2).approve(GBT.address, ten);
        await GBT.connect(user2).buy(ten, 1, 1782282187, AddressZero);
    });
    ...
    it('User3 Buys GBT with 100 WETH', async function () {
        console.log("******************************************************");
        await weth.connect(user3).approve(GBT.address, oneHundred);
        await GBT.connect(user3).buy(oneHundred, 1, 1782282187, AddressZero);
    });

    it('Owner adds User3 to affiliate list', async function () {
        console.log("******************************************************");
        await expect(factory.connect(user3).updateGumBallAffiliate(GBT.address, [user3.address], true)).to.be.reverted;
        await factory.connect(owner).updateGumBallAffiliate(GBT.address, [user3.address], true)
    });

    it('User1 Buys GBT with 10 WETH with user3 affiliate', async function () {
        console.log("******************************************************");
        await weth.connect(user1).approve(GBT.address, ten);
        await GBT.connect(user1).buy(ten, 1, 1782282187, user3.address);
    });
    ...
    it('Artist adds User2 to affiliate list', async function () {
        console.log("******************************************************");
        await expect(GBT.connect(owner).setAffiliate([user2.address], true)).to.be.reverted;
        await GBT.connect(artist).setAffiliate([user2.address], true)
    });
    
    it('User2 transfers GBT and WETH to User1', async function () {
        console.log("******************************************************");
        await weth.connect(user2).transfer(user1.address, await weth.balanceOf(user2.address));
        await GBT.connect(user2).transfer(user1.address, await GBT.balanceOf(user2.address));
    });

    it('User1 Buys GBT with 10 WETH with user2 affiliate', async function () {
        console.log("******************************************************");
        await weth.connect(user1).approve(GBT.address, ten);
        await GBT.connect(user1).buy(ten, 1, 1782282187, user2.address);
    });

    it('Artist removes User3 from affiliate list', async function () {
        console.log("******************************************************");
        await expect(GBT.connect(owner).setAffiliate([user3.address], false)).to.be.reverted;
    ...
    it('Artist readds User3 from affiliate list', async function () {
        console.log("******************************************************");
        await expect(GBT.connect(owner).setAffiliate([user3.address], true)).to.be.reverted;
    ...
    it('User3 Buys GBT with 10 WETH with user1 affiliate', async function () {
        console.log("******************************************************");
        await weth.connect(user3).approve(GBT.address, ten);
        await GBT.connect(user3).buy(ten, 1, 1782282187, user1.address);
    });
